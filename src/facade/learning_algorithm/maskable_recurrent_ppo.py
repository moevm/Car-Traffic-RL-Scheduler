...
class MaskableRecurrentPPO(RecurrentPPO):
    ...
    def collect_rollouts(
        ...
        while n_steps < n_rollout_steps:
            ...
            with th.no_grad():
                ...
                action_mask_tensor = th.as_tensor(self._last_action_mask, device=self.device, dtype=th.float64)
                episode_starts = th.tensor(self._last_episode_starts, dtype=th.float32, device=self.device)
                actions, values, log_probs, lstm_states = self.policy.forward_maskable(obs_tensor, lstm_states,
                                                                                       episode_starts,
                                                                                       action_mask_tensor)
            ...
            self._last_action_mask = np.stack([info["action_mask"] for info in infos])
    ...
    def train(self) -> None:
        ...
        for epoch in range(self.n_epochs):
            ...
            for rollout_data in self.rollout_buffer.get_maskable(self.batch_size):
                actions = rollout_data.actions
                action_masks = rollout_data.action_masks
                if isinstance(self.action_space, spaces.Discrete):
                    # Convert discrete action from float to long
                    actions = rollout_data.actions.long().flatten()
                    action_masks = rollout_data.action_masks.long().flatten()
                # Convert mask from float to bool
                mask = rollout_data.mask > 1e-8

                values, log_prob, entropy = self.policy.evaluate_actions_maskable(
