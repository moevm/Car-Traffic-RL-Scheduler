...
class Facade:
    ...
    def print_traffic_light_info(self):
        print(traci.trafficlight.getRedYellowGreenState('1'), traci.trafficlight.getPhase('1'),
              traci.trafficlight.getPhaseDuration('1'))
        print(traci.trafficlight.getControlledLanes('1'), '\n')
    ...
    def execute(self) -> None:
        ...
        traci.simulationStep()
        k = 0
        sumolib_net = self.__net.get_sumolib_net()
        for node in self.__net.get_nodes():
            if len(sumolib_net.getNode(node).getOutgoing()) > 2:
                k += 1
        print(len(traci.trafficlight.getIDList()), k)
